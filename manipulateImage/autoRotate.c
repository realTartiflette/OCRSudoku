...
int *GetBlankLenght(SDL_Surface *img, int y, int maxlen)
{
...
double autoRotation(SDL_Surface *img)
{
	...
	for(int y = 0; y < img->h; y++)
	{
		for (int x = 0; x < img->w; x++)
		{
			...
			{
				int tmpX = x;
				while(color>150 && (tmpX-x)!=20 && tmpX<img->w)
				{
					GetPixelValue(img,tmpX,y,&color);
					tmpX++;
				}
				if(tmpX!=img->w && (tmpX-x)!=10)
				{
					cornerPixel = createPos((tmpX+x)/2-1, y);
					int *res;
					int newLen = 0;
					int oldLen = -1;
					int i = 0;
					...
					{
						oldLen = newLen;
						res = GetBlankLenght(img, (i+y+1), maxlen+3);
						newLen = res[0];
						//printf("%i\n", newLen);
						i++;
					}
					if(i==20)
					{
						...
						//printf("atan((float)(%i - %i)/(float)((%i-%i)/2))*180/M_PI;\n", leftPixel->b, cornerPixel->b, rightPixel->a,leftPixel->a);

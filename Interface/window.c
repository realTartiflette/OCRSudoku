...
void on_saveFile_clicked(GtkButton *b)
{
	...
	if(saveFile != NULL)
	{
		...
		fclose(saveFile);
		int error = system("mv sudoku.result ~/Downloads/sudoku.result 2> error");
		if (error)
		{
			error = system("mv sudoku.result ~/Téléchargements/sudoku.result 2> error");
			if(error)
				printf("Folder Downloads or Téléchargements not found");
		}
	}
